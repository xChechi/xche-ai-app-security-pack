# AGENTS.md — Security rules (cross-tool)

> The portable, cross-tool version of these rules. Read natively by Codex, Cursor,
> GitHub Copilot, Gemini CLI, Aider, Windsurf, Zed and 25+ other agents.
> Claude Code users: `CLAUDE.md` carries the same rules plus Claude-specific extras.
> Full policy with rationale: see `SECURITY.md`.
>
...
- **Process:** for changes touching auth, payments, secrets, or LLM tools, say so explicitly. Never weaken an existing control to pass a test. Confirm fixes on staging, not prod.
